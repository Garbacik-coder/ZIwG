...
class Recommended(Resource):
    def get(self):
        ...
        def get_recommended(tx, user_id):
            return list(tx.run(f'MATCH (movie:Movie)-[is_predicted:IS_PREDICTED]->(user:User) WHERE user.userId = {user_id} RETURN movie ORDER BY is_predicted.prediction DESC'))

        db = get_db()
        result = db.execute_write(get_recommended, user_id)
...
class Rated(Resource):
    def get(self):
        ...
        def get_rated(tx, user_id):
            return list(tx.run(f'MATCH (user:User)-[rated:RATED]->(movie:Movie) WHERE user.userId = {user_id} RETURN movie'))
